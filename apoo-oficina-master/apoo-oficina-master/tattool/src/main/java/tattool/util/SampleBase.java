...
/**
 */
abstract public class SampleBase  {


	/**
	 * Create a TextArea that automatically saves its contents as a CSS files and
	 * adds that to the stage.
	 * 
	 * @param stage
	 * @return
	 */
	protected TextArea createTextAreaForCSS(Stage stage, ObservableList<String> examples) {
		try {
			...
			lTextArea.setOnMouseClicked((evt) -> {
				...
				lPopup.show(lTextArea, jfxtras.util.NodeUtil.screenX(lTextArea),
						jfxtras.util.NodeUtil.screenY(lTextArea));
			});
	...
	private void applyCSSInTextArea(Stage stage, File file, TextArea textArea) {
		...
		try (FileWriter lFileWriter = new FileWriter(file, false);) {
	...
	protected <T> int addObservableListManagementControlsToGridPane(String label, String tooltip, GridPane gridPane,
			int rowIdx, ObservableList<T> observableList, StringToTypeConverter<T> stringConverter) {
	...
	protected <T> int addObservableListManagementControlsToGridPane(String label, String tooltip, GridPane gridPane,
			int rowIdx, ObservableList<T> observableList, Control textField, ControlToTypeConverter<T> controlConverter,
			TypeToStringConverter<T> toStringConverter) {
		Map<T, String> map = new WeakHashMap<>();
